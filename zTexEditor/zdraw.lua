...
function zdraw:Create(pid, zedre)
	...
	if zedre then
		cout = cout + 1
		zdraw[cout] = {};
		zdraw[cout].arg = CreateDraw(6035, 1690, zedre, "Font_Old_10_White_Hi.TGA", 255,255,255); --CreateDraw(3015, 5035)
...
addCommandHandler({"/zdraw","/здрев"}, CMD_CDRAW);
...
addCommandHandler({"/dinit","/динит"}, CMD_INITTEX);

function DrawSave(playerid)
	zdraw:Save(playerid)
end
addCommandHandler({"/drawsave"}, DrawSave);
...
function zDrawKey(playerid, keydown, keyup)
	if EditorPlayaDraw[playerid].zdrawOn == true then
